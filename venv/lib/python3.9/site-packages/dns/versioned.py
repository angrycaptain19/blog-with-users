...
class WritableVersion(Version):
    def __init__(self, zone, replacement=False):
        ...
        id = zone._versions[-1].id + 1 if len(zone._versions) > 0 else 1
    ...
    def _maybe_cow(self, name):
        ...
        if node is not None and node.id == self.id:
            return node
    ...
    def reader(self, id=None, serial=None):  # pylint: disable=arguments-differ
        ...
        with self._version_lock:
            if id is not None:
                ...
                oname = dns.name.empty if self.relativize else self.origin
                version = None
                for v in reversed(self._versions):
                    n = v.nodes.get(oname)
                    if n:
